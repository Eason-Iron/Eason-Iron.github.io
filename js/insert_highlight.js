(() => {
  ...
  <div class="code-figcaption">
    ...
    <div class="code-right-wrap">
      <div class="code-copy icon-copy" title="Copy code" aria-label="Copy code"></div>
      <div class="icon-chevron-down code-expand" title="Expand or collapse code" aria-label="Expand or collapse code"></div>
    </div>
  </div>`;
  ...
  const getCodeText = (trigger) => {
    const figure = trigger.closest("figure");
    const codeElement =
      figure?.querySelector("td.code") ||
  ...
  const clipboard = new ClipboardJS(".code-copy", {
    text: (trigger) => {
      let selectedText = getCodeText(trigger);
  ...
  clipboard.on("success", (e) => {
    ...
    const successText = getLocalizedText(tips.success, "Copy successfully");
    showCopyTip(successText);
    setTimeout(() => {
  ...
  clipboard.on("error", (e) => {
    ...
    e.trigger.classList.remove("icon-copy");
    const failText = getLocalizedText(tips.fail, "Copy failed");
    showCopyTip(failText);
    setTimeout(() => {
      e.trigger.classList.add("icon-copy");
